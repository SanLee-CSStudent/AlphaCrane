...
Node* Balance::aStarSearch()
{
	...
	while (this->aStarFrontier.size() > 0)
	{
		sameArray = true;
		exploredAlready = 0;
		if (aStarFrontier.size() > 15000 || numExplored > 15000) //Stops at 15000 nodes in the frontier or 15000 nodes explored to prevent impossible search, takes about 6.5 MINUTES
		{
			if (goalQueue.empty())
			{
		...
		for (map<Node*, int>::iterator it = exploredSet.begin(); it != exploredSet.end(); ++it)
		{
			...
			for (int column = 0; column < curr->getData()->getColumn(); column++) //currently doesn't find differences between 
			{
		...
		if (exploredAlready == false || firstRun == true)
		{
			...
			numExplored++;
			//cout << "The best state to expand with g(n) = " << curr->getDepth() << "and h(n) = " << curr->getHCost() << "is ... " << endl;
			//curr->getData()->print();
			...
			{
				goalQueue.push(curr);
			//	cout << endl << "This is a goal node and will not be expanded." << endl;
			}
			else
			{
