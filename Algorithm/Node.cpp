...
void Node::createBalanceChildren() //create a Node* child for every possible move for every moveable container
{
	...
	for (char row = 0; row < this->buffer->getRow(); row++) //loop through buffer grid and add moveable containers to a list
	{
	...
	for (char i = 0; i < bufferContainers.size(); i++) //generate all moves to move containers that are within the buffer and generate a child for every possible move
	{
		...
		{
			if (get<0>(unusedBufferLocations.at(j))+1 == get<0>(originalLoc) && get<1>(unusedBufferLocations.at(j)) == get<1>(originalLoc)) //don't let a container be sat on top of its old position
			{
				continue;
			}
			Node* child = new Node(this, copyData(this->data), copyData(this->goalState), copyData(this->buffer));
...
void Node::createSIFTGoal() //creates the goal state for when performing SIFT (FIRST VERSION COMPLETE)
{
	...
	char col = 7;
	bool goRight = true;
	for (char n = 0; n < containers.size(); n++) //iterate through the list of containers
	{
	...
	cout << endl << "Printing goal state: " << endl << endl;
...
void Node::calcBalanceCost()
{
	...
	for (char i = 0; i < this->data->getRow(); i++) //grabs all containers on left and right side and finds the leftWeight and rightWeight
	{
		for (char j = 0; j < this->data->getColumn(); j++)
		{
			if (this->data->getContainer(j, i).name != "NAN" && this->data->getContainer(j, i).name != "UNUSED")
			{
	...
	if (leftWeight < rightWeight)
	{
		...
		if (this->hCost == 0 || this->hCost == -1) //there isn't one container that can solve balance issue...
		{
			...
			{
				/*
				if (this->data->getContainer(rightContainers.at(i).position.column, (rightContainers.at(i).position.row)).name != "UNUSED") //used to be position.row+1
				{
	...
	else //rightWeight < leftWeight
	{
		...
		if (this->hCost == 0 || this->hCost == -1) //there isn't one container that can solve balance issue...
		{
			...
			char slotsFound = 0;
			vector<int> candidates;
			for (char i = 0; i < leftContainers.size(); i++) //move as few containers as possible to make deficit as small as possible
			{
				/*
				if (this->data->getContainer(leftContainers.at(i).position.column, (leftContainers.at(i).position.row)).name != "UNUSED") //used to be position.row + 1
				{
					continue; //skip this container since it cannot be moved
				}
				*/
	...
	for (char col = 0; col < this->buffer->getColumn(); col++)
	{
		for (char row = 0; row < this->buffer->getRow(); row++)
		{
			...
			{
				//bufferContainers.push_back(this->buffer->getContainer(col, row));
				this->hCost += 15; //add a minimum cost of 15 for every container in buffer
			}
		}
	}
...
void Node::calcSIFTCost() //calculates total distance to SIFT goal state. (FIRST VERSION DONE)
{
	...
	char tempBufferRDist;
	char tempShipRDist;
	for (char row = 0; row < this->data->getRow(); row++) //loop through ship grid and add containers to a list
	{
		for (char col = 0; col < this->data->getColumn(); col++)
		{
			if (this->data->getContainer(col, row).name != "NAN" && this->data->getContainer(col, row).name != "UNUSED")
	...
	for (char row = 0; row < this->buffer->getRow(); row++) //loop through buffer grid and add containers to a list
	{
		for (char col = 0; col < this->buffer->getColumn(); col++)
		{
...
void Node::transferContainer(bool destination, tuple<char, char>destCoords, tuple<char, char> originalCoords, Container c)
{
	if (destination)
	{
...
{
	for (char row = 0; row < this->buffer->getRow(); row++) //loop through ship grid and add moveable containers to a list
	{
		for (char col = 0; col < this->buffer->getColumn(); col++)
		{
			if ((this->buffer->getContainer(col, row).name == "UNUSED") && (col == 3 || this->buffer->getContainer(col + 1, row).name != "UNUSED"))
