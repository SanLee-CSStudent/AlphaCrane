...
void ContainerGrid::addContainer(char columnIndex, char rowIndex, Container c) {
    ...
    grid[columnIndex][rowIndex].position.row = rowIndex; //I added these lines to try to update position of container when moving between ship and container and vice versa
    grid[columnIndex][rowIndex].position.column = columnIndex; //I added these lines to try to update position of container when moving between ship and container and vice versa
}
...
void ContainerGrid::moveContainer(Container c, char columnIndex, char rowIndex) {
    if (!this->checkIndexRange(rowIndex, columnIndex)) {
        // throw an error message and return;
        std::cout << "Row or column index out of range" << std::endl;
        return;
